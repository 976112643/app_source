package com.wq.support.utils.logic;
...
public class ColorArt {
	private final double COLOR_THRESHOLD_MINIMUM_PERCENTAGE = 0.01;// 颜色最低引用数目阈值
	private final double EDGE_COLOR_DISCARD_THRESHOLD = 0.3;// 预选色排除时的最低比率阈值
	private final float MINIMUM_SATURATION_THRESHOLD = 0.15f;
	// private final int BG_SCAN_STEP = 10;
	...
	private int findEdgeColor() {
		...
		if (!sortedColorIterator.hasNext()) { return Color.BLACK; }

		CountedColor proposedEdgeColor = sortedColorIterator.next();// 获取到出现次数最多的颜色,作为预选色
		if (!proposedEdgeColor.isBlackOrWhite()) { return proposedEdgeColor.getColor(); }
		...
		while (sortedColorIterator.hasNext()) {
			...
			// 遍历余下的颜色值,下一个的色值数量与预选色的数量比率小于了预设的 最小比率值,则跳出循环,使用当前预选的色值
			if (edgeColorRatio <= EDGE_COLOR_DISCARD_THRESHOLD) {
				break;
			}

			if (!nextProposedColor.isBlackOrWhite()) {
				// 若当前色值不为白色或黑色,则使用当前色值作为预选色
				proposedEdgeColor = nextProposedColor;
				break;
			}
		}
		// 以上尝试从筛选到的所有色值中取得一个较为中性的色值
	...
	private void findTextColors(HashBag<Integer> colors) {
		...
		int currentColor;
		ArrayList<CountedColor> sortedColors = new ArrayList<CountedColor>();
		boolean findDarkTextColor = !isDarkColor(mBackgroundColor);// 判断背景色是否不是黑色,
		...
		for (CountedColor currentContainer : sortedColors) {
			...
			if (mPrimaryColor == null) {
				...
				if (!isDistinctColor(mPrimaryColor, currentColor) || !isContrastingColor(currentColor, mBackgroundColor)) {
					continue;
				}
				mSecondaryColor = currentColor;
			} else if (mDetailColor == null) {
				if (!isDistinctColor(mSecondaryColor, currentColor) || !isDistinctColor(mPrimaryColor, currentColor) || !isContrastingColor(currentColor, mBackgroundColor)) {
					continue;
				}
	...
	private int colorWithMinimumSaturation(int color, float minSaturation) {
		...
		if (hsv[1] < minSaturation) { return Color.HSVToColor(new float[]
		{ hsv[0], minSaturation, hsv[2] }); }
	...
	private boolean isDistinctColor(int colorA, int colorB) {
		...
		if (Math.abs(r - r1) > threshold || Math.abs(g - g1) > threshold || Math.abs(b - b1) > threshold || Math.abs(a - a1) > threshold) {
			// check for grays, prevent multiple gray colors

			if (Math.abs(r - g) < .03 && Math.abs(r - b) < .03 && (Math.abs(r1 - g1) < .03 && Math.abs(r1 - b1) < .03)) { return false; }
	...
	private class HashBag<K> extends HashMap<K, Integer> {
		private static final long serialVersionUID = 8564889407526762679L;
	...
	private class CountedColor implements Comparable<CountedColor> {
		...
		public boolean isBlackOrWhite() {
			...
			if ((r > .91 && g > .91 && b > .91) || (r < .09 && g < .09 && b < .09))
				return true;
