...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private LoadingLayout mHeaderLayout2;
	protected LoadingLayout mOtherHeaderLayout;
	...
	public final OnRefreshListener2<T> getRefreshListener2() {
		return mOnRefreshListener2;
	}

	public final OnRefreshListener<T> getRefreshListener() {
	...
	public final LoadingLayout getHeaderLayout() {
		return mOtherHeaderLayout == null ? mHeaderLayout : mHeaderLayout2;
	}

	public final void setHeaderLayout(LoadingLayout layout) {
		mHeaderLayout = layout;
	}

	public final void setOtherHeaderLayout(LoadingLayout layout) {
		mOtherHeaderLayout = layout;
	}
	...
	protected void onPullToRefresh() {
		...
		case PULL_FROM_START:
			if (mOtherHeaderLayout != null) {
				mOtherHeaderLayout.pullToRefresh();
			} else mHeaderLayout.pullToRefresh();
			break;
		default:
			// NO-OP
			break;
	...
	protected void onRefreshing(final boolean doScroll) {
		if (mMode.showHeaderLoadingLayout()) {
			if (mOtherHeaderLayout != null) {
				mOtherHeaderLayout.refreshing();
			} else mHeaderLayout.refreshing();
		}
	...
	protected void onReleaseToRefresh() {
		...
		case PULL_FROM_START:
			if (mOtherHeaderLayout != null) {
				mOtherHeaderLayout.releaseToRefresh();
			} else mHeaderLayout.releaseToRefresh();
			break;
		default:
			// NO-OP
			break;
	...
	protected void onReset() {
		...
		if (mOtherHeaderLayout != null) {
			// mOtherHeaderLayout.reset();
		} else mHeaderLayout.reset();
		mFooterLayout.reset();
		...
		if (mLayoutVisibilityChangesEnabled) {
			if (value < 0) {
				if (mOtherHeaderLayout == null)
					...
					mOtherHeaderLayout.setVisibility(View.VISIBLE);
				}
			} else if (value > 0) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mHeaderLayout2 = createLoadingLayout(context, Mode.PULL_FROM_START, a);
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			default:
				if (mOtherHeaderLayout != null) {
					mOtherHeaderLayout.onPull(scale);
				} else mHeaderLayout.onPull(scale);
				break;
			}
